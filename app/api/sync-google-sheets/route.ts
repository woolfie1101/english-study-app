  ...
  try {
    ...
    for (const row of pendingRows) {
      try {
        ...
        console.log(`✅ Finished processing row ${row.get('number')}`);

        // Skip Google Sheets status update due to API hanging issue
        // Data is already saved to Supabase successfully
        // Note: Google Sheets status will remain as pending/empty, but Supabase is the source of truth
        syncedCount++;
      } catch (error) {
    ...
    console.log(`✅ Successfully synced ${syncedCount} rows`);
